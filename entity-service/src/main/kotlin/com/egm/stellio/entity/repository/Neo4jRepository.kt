...
class Neo4jRepository(
    ...
    fun createRelationshipOfSubject(
        ...
        val resultForExistence = session.query(queryForTargetExistence, parametersForExistence).toList()
        val targetAlreadyExists = resultForExistence.isNotEmpty()
        ...
        val query =
            if (targetAlreadyExists)
                ...
                """
                MATCH (subject:${subjectNodeInfo.label} { id: ${'$'}subjectId })
                MERGE (target:PartialEntity {
                            id: ${'$'}targetId,
                            createdAt: datetime("${Instant.now().atZone(ZoneOffset.UTC)}")
                })
                CREATE (subject)-[:HAS_OBJECT]->(r:Attribute:Relationship:`${relationship.type[0]}` ${'$'}props)
                        -[:$relationshipType]->(target)
                """.trimIndent()
